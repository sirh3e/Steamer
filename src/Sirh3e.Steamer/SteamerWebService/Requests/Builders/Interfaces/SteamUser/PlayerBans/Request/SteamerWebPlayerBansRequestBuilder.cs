...
namespace Sirh3e.Steamer.SteamerWebService.Requests.Builders.Interfaces.SteamUser.PlayerBans.Request
{
    public class SteamerWebPlayerBansRequestBuilder : SteamerRequestBuilder<SteamerWebPlayerBansRequestBuilder, ISteamerWebPlayerBansRequest>,
                                                      ISteamerWebPlayerBansRequestBuilder //ToDo rename SteamerMethod
    {
        ...
        public ISteamerWebPlayerBansRequestBuilder SetSteamIds(params ulong[] steamIds)
            => SetValue("steamids",
                        string.Join(",",
                                    steamIds.Select(steamId => steamId.ToString()).ToList() ?? throw new ArgumentNullException(nameof(steamIds))));
